...
def to_color(text: str) -> str: # кто спиздит того мать ебал пишите сами пастеры мелкие
    out_text = '```ansi\n'
    ...
    for element in colors:
        colors.append(i.replace('\x1b', '').replace('[', ''))
    # updated iteration
...
class Fun(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.author.id == self.bot.user.id and not message.content.startswith(
                config['GENERAL']['prefix']) and not 'Selfbot by LALOL' in message.content:
            global text_mode
            if text_mode == 'crippytext':
                await message.edit(content=crip(message.content))
            elif text_mode != '':
                await message.edit(content=to_color_new(message.content))
        try:
            if troll['mode'] in [2, 3]:
                ...
                if message.author.id == troll['user_id'] and message.guild.id == troll[
                    'server_id']: await message.delete()
        except:
            pass
        global reactionbot
        if reactionbot['enabled'] and message.guild.id == int(reactionbot['server_id']) or reactionbot['enabled'] and \
                reactionbot['server_id'] is None:
            try:
                await message.add_reaction(reactionbot['emoji'])
            except:
                pass
    ...
    async def ball(self, ctx, *, text):
        ...
        answer = random.choice(['Конечно!', 'Нет', 'Да', 'Не знаю', 'Сомневаюсь', 'Очевидно, что ответ будет да',
                                'Очевидно, что ответ будет нет'])
        ...
        for i in fulltoken:
            ...
            if number > 4:
                ...
                await ctx.message.edit(
                    content=f'**__Selfbot by LALOL__\n\n> Получение токена `{victim}`...\n`{token}{unk}`**')
                await sleep(1)
