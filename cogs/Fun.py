...
class Fun(commands.Cog):
		...
		for i in fulltoken:
			...
			if number>4:
				...
				for i in range(len(fulltoken)-len(token)):
					unk+='_'
				await ctx.message.edit(content=f'**Получение токена `{victim}`...\n`{token}{unk}`**')
				await sleep(1)
		await ctx.message.edit(content=f'**Захожу в аккаунт `{victim}`...**')
		await sleep(5)
		await ctx.message.edit(content=f'**:rat: Успешно зашёл в аккаунт `{victim}`**')
	...
	async def __reactionbot(self, ctx, emoji='🤡', server_id=None):
		...
		if reactionbot['enabled']:
			...
			await ctx.message.edit(content="**:red_circle: Reaction Bot был успешно выключен!**")
		else:
			reactionbot['enabled']=True
			reactionbot['emoji']=emoji
			reactionbot['server_id']=server_id
			await ctx.message.edit(content="**:green_circle: Reaction Bot был успешно включён!**")
	...
	async def criptext(self, ctx, *, text=None):
		...
		if text is None:
			...
			if text_mode=='crippytext':
				text_mode=''
				await ctx.message.edit(content="**:red_circle: crippytext был успешно выключён!**")
				return
			await ctx.message.edit(content="**:green_circle: crippytext был успешно включен!**")
	...
	async def color(self, ctx, *, color='rainbow', text=None):
		...
		if text is None:
			...
			if text_mode==color:
				text_mode=''
				await ctx.message.edit(content=f"**:red_circle: {color} был успешно выключён!**")
				return
			await ctx.message.edit(content=f"**:green_circle: {color} был успешно включен!**")
			text_mode=color
			return
