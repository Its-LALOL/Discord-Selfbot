...
class Moderation(commands.Cog):
	...
	async def ban(self, ctx, user:discord.User, *, reason='Причина не указана'):
		try: await ctx.guild.ban(user, reason=reason)
			...
			await ctx.message.edit(content=f'**:x: Не удалось забанить пользователя `{user}`**')
			return
		await ctx.message.edit(content=f'**:dagger: Пользователь `{user}` успешно был забанен по причине `{reason}`**')
	@commands.command(aliases=['разбанить', 'разбан'])
	async def unban(self, ctx, user:discord.User):
		try: await ctx.guild.unban(user)
		except:
			await ctx.message.edit(content=f'**:x: Не удалось разбанить пользователя `{user}`**')
			return
		await ctx.message.edit(content=f'**:ok_hand: Пользователь `{user}` успешно был разбанен**')
	...
	async def kick(self, ctx, user:discord.Member, *, reason='Причина не указана'):
		try: await ctx.guild.kick(user, reason=reason)
			...
			await ctx.message.edit(content=f'**:x: Не удалось кикнуть пользователя `{user}`**')
			return
		await ctx.message.edit(content=f'**:door: Пользователь `{user}` успешно был кикнут по причине `{reason}`**')
	...
	async def mute(self, ctx, user:discord.Member, time, *, reason='Причина не указана'):
		if time.endswith('s') or time.endswith('с'):
			...
			await ctx.message.edit(content=f'**:x: Вы неправильно указали длительность мута!**')
			return
		...
		if response==403:
			await ctx.message.edit(content=f'**:x: Не удалось замутить пользователя `{user}`**')
			return
		await ctx.message.edit(content=f'**:mute: Пользователь `{user}` был успешно замучен по причине `{reason}` на `{time}`**')
	...
	async def unmute(self, ctx, user:discord.Member, *, reason='Причина не указана'):
		...
		if response==403:
			await ctx.message.edit(content=f'**:x: Не удалось размутить пользователя `{user}`**')
			return
		await ctx.message.edit(content=f'**:sound: Пользователь `{user}` был успешно размучен по причине `{reason}`**')
	...
	async def slowmode(self, ctx, time):
		if time.endswith('s') or time.endswith('с'):
			...
			await ctx.message.edit(content=f'**:x: Длительность слоумода указана неправильно!**')
			return
		if seconds>21600:
			await ctx.message.edit(content=f'**:x: Слоумод не может быть больше 6 часов!**')
			return
		await ctx.channel.edit(slowmode_delay=seconds)
		await ctx.message.edit(content=f'**:timer: Слоумод канала был успешно изменён на {seconds} секунд**')
